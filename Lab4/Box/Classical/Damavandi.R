# Определение функции Damavandi
# Параметры:
#   x: Входной вектор
# Возвращает:
#   Значение функции Damavandi в заданной точке
Damavandi <- function(x) {
  ...
  # Вычисление значения функции Damavandi
  y <- (1 - abs((sin(pi * (x[, 1] - 2))) * (sin(pi * (x[, 2] - 2))) / (pi^2 * (x[, 1] - 2) * (x[, 2] - 2)))^5) *
    (2 + (x[, 1] - 7)^2 + 2 * (x[, 2] - 7)^2)
