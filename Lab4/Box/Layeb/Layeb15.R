Layeb15 <- function(x) {
  ...
  #   Недифференцируемая, несепарабельная, масштабируемая, многомодальная,
  ...
  if (missing(x)) {
    ...
    y$features <- c(0, 0, 1, 1, 0, 0, 0, 0)  # Вектор, описывающий свойства задачи
    ...
    for (i in 1:(dim - 1)) {
      ...
      y <- y + 10*abs(tanh((2*abs(x[i]) - x[i + 1]^2 - 1)))^0.5 + abs(exp(x[i + 1]*x[i] + 1) - 1)
    }
