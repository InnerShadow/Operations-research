...
AttractiveSectorBBOB <- function(x) {
  if (missing(x)) {
    y <- list(
      nx = 0,
      ng = 0,
  ...
  fopt <- get_fmin(dim)
  funid <- 6
  scales <- diag(sqrt(10) ^ seq(0, 1, length.out = dim))
  linearTF <- (compute_rotation(6, dim) %*% scales) %*% compute_rotation(6 + 1e+6, dim)
  
  z <- (t(x_shift(x, xopt)) %*% linearTF)
  idx <- which(z * xopt > 0)
  z[idx] <- 100 * z[idx]
  y <- toz(sum(z^2))^(0.9) + get_fmin(dim)
